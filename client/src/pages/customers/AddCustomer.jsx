...
const AddCustomer = ({ newCustomer, customers, closeEvent }) => {
  // Initial Values
  const initialValues = {
    name: "",
    gender: "",
  ...
  const onSubmit = async (formData, onSubmitProps) => {
    try {
      await axios.post(`/api/v1/customer`, formData, { headers: { accessToken: localStorage.getItem("accessToken") } });
      setTimeout(() => {
              ...
              <Grid container spacing={2}>
                ...
                <Grid item xs={12}>
                  ...
                  <Button type="submit" label="Submit" disabled={!(formik.dirty && formik.isValid) || formik.isSubmitting}>
                    {formik.isSubmitting ? "Loading" : "Create Customer"}
                  </Button>
                </Grid>
              </Grid>
            </Form>
