...
const Expenses = () => {
  ...
  const getExpenses = async () => {
    setIsLoading(true);
    try {
      await axios.get(`/api/v1/expense`, { headers: { accessToken: localStorage.getItem("accessToken") } }).then((res) => {
  ...
  const handleView = async (id) => {
    try {
      await axios.get(`/api/v1/expense/${id}`, { headers: { accessToken: localStorage.getItem("accessToken") } }).then((res) => {
  ...
  const handleDelete = async (id) => {
    return await Swal.fire({
      ...
      if (res.value) {
        try {
          await axios.delete(`/api/v1/expense/${id}`, { headers: { accessToken: localStorage.getItem("accessToken") } });
          getExpenses();
