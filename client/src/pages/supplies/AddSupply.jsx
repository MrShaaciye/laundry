...
const AddSupply = ({ newSupply, closeEvent }) => {
  // Initial Values
  const initialValues = {
    name: "",
    note: "",
  ...
  const onSubmit = async (formData, onSubmitProps) => {
    try {
      await axios.post(`/api/v1/supply`, formData, { headers: { accessToken: localStorage.getItem("accessToken") } });
      setTimeout(() => {
      ...
      newSupply(formData);
      closeEvent();
      return toast.success(`Supply added successfully`);
    } catch (err) {
      closeEvent();
              ...
              <Grid container spacing={2}>
                ...
                <Grid item xs={12}>
                  ...
                  <Button type="submit" label="Submit" disabled={!(formik.dirty && formik.isValid) || formik.isSubmitting}>
                    {formik.isSubmitting ? "Loading" : "Create Supply"}
                  </Button>
                </Grid>
              </Grid>
            </Form>
