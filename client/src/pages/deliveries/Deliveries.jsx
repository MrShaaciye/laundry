...
const Inventories = () => {
  ...
  const getCustomers = async () => {
    try {
      axios.get(`/api/v1/customer`, { headers: { accessToken: localStorage.getItem("accessToken") } }).then((res) => {
  ...
  const getEmployees = async () => {
    try {
      axios.get(`/api/v1/employee`, { headers: { accessToken: localStorage.getItem("accessToken") } }).then((res) => {
  ...
  const getDeliveries = async () => {
    setIsLoading(true);
    try {
      await axios.get(`/api/v1/delivery`, { headers: { accessToken: localStorage.getItem("accessToken") } }).then((res) => {
  ...
  const handleView = async (id) => {
    try {
      await axios.get(`/api/v1/delivery/${id}`, { headers: { accessToken: localStorage.getItem("accessToken") } }).then((res) => {
  ...
  const handleDelete = async (id) => {
    return await Swal.fire({
      ...
      if (res.value) {
        try {
          await axios.delete(`/api/v1/delivery/${id}`, { headers: { accessToken: localStorage.getItem("accessToken") } });
          getDeliveries();
