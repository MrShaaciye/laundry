...
const Inventories = () => {
  ...
  const getSupplies = async () => {
    try {
      axios.get(`/api/v1/supply`, { headers: { accessToken: localStorage.getItem("accessToken") } }).then((res) => {
  ...
  const getInventories = async () => {
    setIsLoading(true);
    try {
      await axios.get(`/api/v1/inventory`, { headers: { accessToken: localStorage.getItem("accessToken") } }).then((res) => {
  ...
  const handleView = async (id) => {
    try {
      await axios.get(`/api/v1/inventory/${id}`, { headers: { accessToken: localStorage.getItem("accessToken") } }).then((res) => {
  ...
  const handleDelete = async (id) => {
    return await Swal.fire({
      ...
      if (res.value) {
        try {
          await axios.delete(`/api/v1/inventory/${id}`, { headers: { accessToken: localStorage.getItem("accessToken") } });
          getInventories();
