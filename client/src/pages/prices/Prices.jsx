...
const Prices = () => {
  ...
  const getPrices = async () => {
    setIsLoading(true);
    try {
      await axios.get(`/api/v1/price`, { headers: { accessToken: localStorage.getItem("accessToken") } }).then((res) => {
  ...
  const handleView = async (id) => {
    try {
      await axios.get(`/api/v1/price/${id}`, { headers: { accessToken: localStorage.getItem("accessToken") } }).then((res) => {
  ...
  const handleDelete = async (id) => {
    return await Swal.fire({
      ...
      if (res.value) {
        try {
          await axios.delete(`/api/v1/price/${id}`, { headers: { accessToken: localStorage.getItem("accessToken") } });
          getPrices();
