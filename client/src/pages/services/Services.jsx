...
const UpdateService = lazy(() => import("./UpdateService"));
const ViewService = lazy(() => import("./ViewService"));
...
const Services = () => {
  ...
  // Get Services from API function
  const getServices = async () => {
    setIsLoading(true);
    try {
      await axios.get(`/api/v1/service`, { headers: { accessToken: localStorage.getItem("accessToken") } }).then((res) => {
  ...
  // Create function
  const handleCreate = () => getServices();

  // View function
  const handleView = async (id) => {
    try {
      await axios.get(`/api/v1/service/${id}`, { headers: { accessToken: localStorage.getItem("accessToken") } }).then((res) => {
  ...
  const handleDelete = async (id) => {
    return await Swal.fire({
      ...
      if (res.value) {
        try {
          await axios.delete(`/api/v1/service/${id}`, { headers: { accessToken: localStorage.getItem("accessToken") } });
          getServices();
    ...
    <>
      ...
      <Stack>
        ...
        <Modal keepMounted open={modalViewOpen} aria-labelledby="responsive-modal-title" aria-describedby="responsive-modal-description">
          <Box sx={style}>
            <ViewService closeEvent={handleViewClose} service={service} />
          </Box>
        </Modal>
      </Stack>
      <Paper sx={{ width: "100%", overflow: "hidden" }}>
        ...
        {/* Search Box */}
        <Stack ml={1} mr={1} direction="row" spacing={2}>
          <TextField type="search" size="small" sx={{ width: 300 }} onChange={(e) => setSearch(e.target.value)} label={`${count} Search Service...`} />
          {/* <Autocomplete disablePortal id="combo-box-demo" size="small" options={services} sx={{ width: 300 }} onChange={v => filterService(v)} getOptionLabel={services => services.name || ""} renderInput={params => <TextField {...params} label={`${count} Search Service...`} />} /> */}
                    ...
                    <TableRow key={service.id}>
                      ...
                      <TableCell>
                        <Stack direction="row" justifyContent="center" alignItems="center" spacing={0.3}>
                          ...
                          <EditIcon style={{ fontSize: "20px", color: "blue", cursor: "pointer" }} onClick={() => handleEdit(service.id)} />
                          <DeleteIcon style={{ fontSize: "20px", color: "red", cursor: "pointer" }} onClick={() => handleDelete(service.id)} />
                        </Stack>
                      </TableCell>
                    </TableRow>
                  ))
