...
const AddService = ({ newService, services, closeEvent }) => {
    // Initial Values
    const initialValues = {
        name: "",
    };
    ...
    const onSubmit = async (formData, onSubmitProps) => {
        try {
            const { name } = formData;
            const serviceExist = services.find(service => service.name === name);
            if (serviceExist) {
                closeEvent();
                return toast.error(`Service already exists`);
            }
            await axios.post(`/api/v1/service`, formData);
            setTimeout(() => {
                            ...
                            <Grid container spacing={2}>
                                ...
                                <Grid item xs={12}>
                                    ...
                                    <Button type="submit" label="Submit" disabled={!(formik.dirty && formik.isValid) || formik.isSubmitting}>
                                        {formik.isSubmitting ? "Loading" : "Create Service"}
                                    </Button>
                                </Grid>
                            </Grid>
                        </Form>
